...
- 트리거:
  - 자동: PR + main push, 단 path-filter (`aggregation/**`, `pyproject.toml`, `.github/workflows/ci.yml`)
  - 수동: `workflow_dispatch`. Actions UI 또는 `gh workflow run`으로 임의 ref에서 즉시 실행 가능. path-filter에 안 걸리는 변경(예: docs-only PR)을 손으로 한 번 돌려보고 싶을 때 사용
- 잡 1 — `python`:
  - `ruff check .`
  - `ruff format --check .`
  - `pytest`
